...
# Official GPU metrics exporter for Prometheus
...
# Stop existing DCGM exporter if running
if docker ps -a | grep -q dcgm-exporter; then
    print_warning "Stopping existing DCGM exporter..."
    docker stop dcgm-exporter 2>/dev/null || true
    docker rm dcgm-exporter 2>/dev/null || true
fi

# Pull DCGM exporter image
print_msg "Pulling NVIDIA DCGM Exporter image..."
...
sleep 5
if curl -s http://localhost:${DCGM_PORT}/metrics | grep -q "DCGM_FI_DEV_GPU_TEMP"; then
    print_msg "Metrics endpoint is working!"
else
    print_warning "Metrics endpoint may not be responding yet. Check logs: docker logs dcgm-exporter"
fi
...
echo "  Logs:    docker logs -f dcgm-exporter"
echo ""
